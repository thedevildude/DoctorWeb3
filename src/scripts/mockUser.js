...
module.exports = async function (callback) {
  ...
  try {
    ...
    const contractAddress = DoctorWeb3.networks[chainId].address;
    ...
    for (let i = 0; i < DoctorArray.length; i++) {
      const tx = await doctorWeb3.ApplyForVerification(
        DoctorArray[i].name,
        DoctorArray[i].address,
        DoctorArray[i].medicalId,
        0
      );
      tx.wait();
    ...
    for (let i = 0; i < HospitalArray.length; i++) {
      const tx = await doctorWeb3.ApplyForVerification(
        HospitalArray[i].name,
        HospitalArray[i].address,
        HospitalArray[i].medicalId,
        1
      );
      tx.wait();
